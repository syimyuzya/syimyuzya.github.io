...
// 分則 2
if ((轉 === '外' && is('一等')) || is('江韻 入聲 幫莊組')) {
  if (尾 === 'k' || (尾 === 'n' && is('合口')) || (尾 === 'u' && is('幫組'))) {
    元 = 'å';
  }
}
...
if (母 === '') {
  ...
  if (lookup.hasOwnProperty(介)) {
    介 = lookup[介];
  } else if (介 === '' && lookup.hasOwnProperty(元)) {
    介 = lookup[元];
  }
}
...
if (options.標調) {
  const 聲 = is('上聲 並定澄從邪崇俟常船羣匣母') ? '去' : 音韻地位.聲;
  const 陰陽 = (聲 === '上' || is('幫滂端透知徹精清心莊初生章昌書見溪影曉母'))
    ? '陰' : '陽';
  const 調號 = {
    平: ['\u0304', '\u0300'],
    上: ['\u030c'],
